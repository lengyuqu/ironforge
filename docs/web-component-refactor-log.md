...
- ⬜ 剩余可选：admin/runners（289）、packages/upload（278）、releases/edit（265，ReleaseForm 可复用）；websockets.ts 2 处 any（实时通道专项）；M2 里程碑 UI（待后端协同确认）；后端协同项：list_reviews/TimeEntry 补 username enrich、AI API 前端入口。
