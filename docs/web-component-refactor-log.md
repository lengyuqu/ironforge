...
- ✅ 质量补强收官批（Q-17 起，2026-09-03，共 12 个提交）：**118/118 组件均有对应单测（缺测 0 / 孤儿测试 0），vitest 197 → 534 用例，组件单测收官**。补齐 FileEditor 与 repo/issues/pipelines/pulls 最后 34 个组件，四目录分别 11/13/5/12 全覆盖：
  - `900df07` fix：7 个 modal 内层容器补 stopPropagation——admin/AuditDetailModal、UserDeleteModal、UserEditModal、labels/LabelDeleteModal、LabelFormModal、milestones/MilestoneDeleteModal、MilestoneFormModal 点表单内容即冒泡到 overlay 误关弹窗（由本批单测"点内部元素 onClose 零调用"断言锁定）；
  - `c46efd1` 测试基建 + components 根目录 10 文件 37 例：vitest.config 补 `$app/environment` 别名、test-stubs 桩、CloneModalStub；Button/Dropdown/Layout/Modal 四 Harness 转发 snippet children——ModalHarness 必须带 onClose 默认转发，否则 Escape/× 抛 `$$props.onClose is not a function` 且用例"假绿"（Tests passed 但 Errors 2）；197 → 234；
  - `f0b001d` admin 目录 11/11（37 例）：AdminUserTable/AuditDetailModal/AuditFilters/AuditLogTable/InstanceSettingsSection/LoginAttemptsSection/RunnerDeleteModal/RunnerRegisterForm/SsoProviderSection/UserDeleteModal/UserEditModal，234 → 273；
  - `e7e48b9` 8 个目录 20/20（58 例）：home 6 + dashboard 2 + orgs 2 + imports 2 + packages 2 + releases 2 + search 1 + wiki 3，273 → 331；
  - `f5c0689` boards/labels/milestones 3 目录 7/7（25 例）：BoardColumn 拖放契约（同列忽略/跨列走 API）/BoardCreateForm/ColumnCreateForm/LabelDeleteModal/LabelFormModal/MilestoneDeleteModal/MilestoneFormModal（本地日期转 RFC 3339、编辑保留 closed 态），331 → 356；
  - `c5e49b0` docs：组件测试收官批 Agent 交接提示词（环境自检基线 356 + 假绿陷阱 + TS 报错四件套 + 全绿三要素定义）；
  - `513d5f8` repo 目录 11/11（30 例）：BlobContentView（markdown 渲染分支）/BlobDeletePanel（conflict 专属提示 vs 通用错误）/BlobFileHeader（复制路径/blob 链接）/CommitInfoCard（GPG 三态徽章）/EmptyRepoGuide（SSH URL 按浏览器 host 拼）/RepoToolbar/StatusChecksPanel，356 → 386；
  - `94e7e8d` FileEditor（722 行、本轮最大交互组件，10 例）：create/edit 初始化、脏检测禁存、payload trim 且 sha 仅 edit+initialSha 携带、必填与四类非法路径拦截、preview/diff tab（textarea 在 diff tab 卸载，需切回 edit tab 再输入）、conflict 专属 reload 入口、disabledReason 禁存禁输入，386 → 396；
  - `e8a9683` fix：收官批单测暴露 3 处真实生产缺陷——①IssueCreateForm 收集 assignees 输入却硬编码 undefined 传 issues.create 第 6 参（新建 issue 指派人人静默丢失）；②ArtifactsPanel $effect 读自身写入的 loading/wasTerminal（$state）→ 无限拉取循环（裸探针可拖垮 vitest worker），记账变量改非响应式 let，按"pipelineId 切换 / 首达终态"触发；③PrThreads $effect 无条件重赋值 selectedSuggestionIds 自引用死循环（5 线程用例各卡约 8.7s），改为长度/逐元素变更检测后才重赋值；
  - `7a1be99` issues 目录 13/13（46 例）：AssigneesPanel/CommentCard/IssueCommentForm/IssueCreateForm（锁死死输入缺陷）/IssueHeader/IssueMilestonePanel（清空传 null）/IssueSelector/IssueTemplateChooser/ReactionBar/TimeEntryForm/TimeEntryList，396 → 442；
  - `481f41b` pipelines 目录 5/5（34 例）：ArtifactsPanel（"挂载只拉一次"锁死 effect 循环）/JobLogModal（WS 流徽标/三关闭路径先 disconnect/卸载 disconnect/Enter-Space 关闭）/PipelineDetailPanel/PipelineFlow（键盘可达、按钮 stopPropagation）/PipelineList，442 → 476；
  - `82a0b9d` pulls 目录 12/12（58 例）：SuggestionBlock/PrHeader（修正 convert_draft/draft 分支误判）/PrTimeline/PrReviewForm/PrReviewList/PrMergeBox/PrReviewersBox/PrDiffView（R 侧行内评论 payload）/PrThreads/PullCreateForm，476 → 534；
  - 收官批坑沉淀：happy-dom 不实现 option 的 :checked（select 绑定只认首 option，排首位后裸 fireEvent.change）；PrDiff/MergeQueueEntry 在 entities 与 api/pulls 重复且形状不同，统一从 `$lib/api/pulls` 导入；events/target 等 prop 名撞 testing-library 挂载选项须 `render(C, { props })` 包裹；重复文本断言用 getAllByText。
- ✅ **收官全量验证：vitest 534/534（127 files，Errors 0）· svelte-check 0 errors / 47 warnings（基线不变）· vite build ✔**。组件单测全程累计暴露生产缺陷 5 类并全部修复：Pagination keyed each 双省略号 key 重复（Q-8，`e401e04`）、7 个 modal 冒泡误关（`900df07`）、IssueCreateForm 死输入 / ArtifactsPanel / PrThreads effect 死循环（`e8a9683`）。
- ⬜ 剩余可选（均为后端依赖项，非组件层）：M2-2（issue 列表里程碑筛选/进度计数，依赖后端 A1/A2 确认）；后端协同项：list_reviews/TimeEntry 补 username enrich、里程碑删除级联策略 A5。组件层 118/118 全覆盖已收官（见上 Q-17 起），后续仅随组件变更回归维护。R4 后 routes/ 250+ 行页面仅剩 login/search 等单功能低频页，拆分系列正式收官；API any 清零收官。
