...
> 目标：把当前约 68%–72% 的细节完成度提升到 90% 左右，优先覆盖小团队真实使用路径。
...
| 里程碑 | 目标 | 预计投入 | 目标完成度 |
|---|---|---:|---:|
| M0：基线与协议正确性 | 消除能力误报，建立真实 Git 客户端回归 | 2–3 周 | 74% |
| M1：仓库与 Issue 高频体验 | 补齐日常协作中最明显的缺口 | 6–8 周 | 80% |
| M2：CI/Actions 兼容 | 从 YAML 转换器提升为可迁移的 Actions 子集 | 8–12 周 | 85% |
| M3：Package 与 API 生态 | 补主流协议、治理能力和高频 Gitea API | 8–12 周 | 88% |
| M4：身份、权限与管理细节 | 补认证源、用户关系和仓库 Unit 权限 | 6–9 周 | 90% |
| M5：生产化 | 对象存储、队列、完整备份、升级与性能 | 10–14 周 | 90%+ 可替代性 |

以上为人力串行估算；3–4 人可并行，但协议、数据模型和基础设施任务不能简单按人数线性压缩。
...
| ID | 状态 | 优先级 | 任务 | 依赖 | 估算 |
|---|---|---|---|---|---:|
| ALIGN-001 | READY | P0 | 建立代码可验证的 Gitea 对齐矩阵，废止旧 CSV 中已过时状态 | 无 | 1–2 天 |
| GIT-001 | TODO | P0 | 正确实现 Protocol V2 shallow/deepen；若暂不支持则停止广告能力 | ALIGN-001 | 3–5 天 |
| GIT-002 | TODO | P1 | 实现 partial clone/filter，并用真实 Git 客户端验证 blob/tree filter | GIT-001 | 4–7 天 |
| GIT-003 | TODO | P1 | 增加 HTTP/SSH、V1/V2、clone/fetch/push、shallow/partial 兼容矩阵 | GIT-001 | 3–5 天 |
...
| ID | 状态 | 优先级 | 任务 | 依赖 | 估算 |
|---|---|---|---|---|---:|
| REPO-101 | TODO | P1 | Push Mirror：多目标、凭证加密、手动/定时/按 push 同步、错误状态 | M0 | 5–8 天 |
| REPO-102 | TODO | P1 | `git archive --remote` / upload-archive，覆盖 HTTP 与 SSH 权限 | M0 | 4–6 天 |
...
| ISSUE-101 | TODO | P1 | Markdown Issue/PR Template 与模板选择页 | 无 | 4–6 天 |
| ISSUE-102 | TODO | P1 | YAML Issue Form：字段校验、预填充、权限与兼容文件位置 | ISSUE-101 | 5–8 天 |
| ISSUE-103 | TODO | P1 | Issue/PR/评论附件上传、下载、删除和存储配额入口 | STORAGE-001 或本地存储接口 | 5–8 天 |
| ISSUE-104 | TODO | P1 | Issue/PR/评论 Reaction，含唯一性、计数和通知策略 | 无 | 3–5 天 |
| ISSUE-105 | TODO | P1 | 多 Assignee 数据模型、API、筛选和 UI | 无 | 4–6 天 |
...
| ID | 状态 | 优先级 | 任务 | 依赖 | 估算 |
|---|---|---|---|---|---:|
| CI-200 | READY | P0 | ADR：确定第三方 Action 的执行模型、信任边界、网络/Secret 策略和兼容范围 | 无 | 2–3 天 |
...
| ID | 状态 | 优先级 | 任务 | 依赖 | 估算 |
|---|---|---|---|---|---:|
| STORAGE-001 | TODO | P0 | 统一 BlobStorage trait，覆盖 LFS/Package/OCI/Artifact/附件/归档 | 无 | 8–12 天 |
...
不要按文档章节完全串行。建议前 12 个开发任务依次为：

1. `ALIGN-001`：冻结真实基线；
2. `GIT-001`：修正 shallow/deepen；
3. `GIT-003`：把 Git 真实客户端矩阵放入 CI；
4. `CI-200`：确定 Actions 执行与安全边界；
5. `STORAGE-001`：统一 BlobStorage，冻结后续附件和备份的存储边界；
6. `ISSUE-101`：Issue/PR Markdown Template；
7. `ISSUE-103`：附件上传、下载与权限闭环；
8. `ISSUE-105`：多 Assignee；
9. `ISSUE-104`：Reaction；
10. `REPO-101`：Push Mirror；
11. `CI-201`：失败 Job 重跑；
12. `OPS-501`：完整实例备份恢复。
...
- 旧 `gitea-gap-list.csv` 不再作为当前事实源，迁移有效条目后归档或重建；
- `ALIGN-001` 完成后立即进入 `GIT-001`，不继续做无代码产出的泛化分析。
