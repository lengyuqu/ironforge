...
| Web UI (SvelteKit)     | ⏳ 未开始 | 规划中 |
| Wiki + LFS             | ⏳ Phase 4 | 规划中 |
| CI/CD 引擎             | ⏳ Phase 5 | 规划中 |
| 仓库权限（git 协议鉴权） | ⏳ 未开始 | 需接入 can_read/can_write |
...
| `rg-core` | 业务逻辑：用户认证（argon2+JWT）、仓库、Issue、Pull Request | ✅ Phase 1-3 |
| `rg-db` | SeaORM 实体 + 迁移（users/repos/ssh_keys/issues/comments/pulls/milestones） | ✅ Phase 1-3 |
...
- Wiki 引擎（`.wiki.git` 裸仓库后端 + Markdown 渲染）
- Git LFS 协议实现（对象存储 + 批量 API）
- 仓库权限：git 协议入口接入 can_read/can_write
- Webhook / 邮件通知

### ⏳ Phase 5 — CI/CD + Web UI
