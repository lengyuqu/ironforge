...
export const issues = {
  ...
  list: (owner: string, repo: string, state?: string, page?: number, perPage?: number, labels?: string, assignee?: string, milestone?: string) => {
    return request<PaginatedResponse<IssueResponse>>(`/repos/${owner}/${repo}/issues${qs({ state, page, per_page: perPage, labels, assignee, milestone })}`)
      .then((response) => ({
        ...response,
